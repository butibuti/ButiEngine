...
void ButiEngine::ScoreUI::Start()
{
	...
	shp_color = ObjectFactory::Create<CBuffer_Dx12<LightVariable>>(3);

	drawInfo->vec_exCBuffer.push_back(shp_color);

	auto light = LightVariable();
	light.lightDir = Vector4(Vector3(1.0f, 1.0f, 1.0f), 1);
	shp_color->SetExName("LightBuffer");
	shp_color->Get() = light;
	auto meshTag = container->GetMeshTag("Plane_UV");
	auto shaderTag = container->GetShaderTag("TextMesh");
...
void ButiEngine::ScoreUI::SetColor(const Vector4& arg_color)
{
	shp_color->Get().lightDir = arg_color;
}
...
void ButiEngine::RemainUI::Start()
{
	int width = 50;
	auto container = gameObject.lock()->GetResourceContainer();
	auto drawInfo = ObjectFactory::Create< DrawInformation>();
...
void ButiEngine::RemainUI::OnUpdate()
{
}

void ButiEngine::RemainUI::SetColor(const Vector4& arg_color)
{
	shp_color->Get().lightDir = arg_color;
}

std::shared_ptr<ButiEngine::GameComponent> ButiEngine::RemainUI::Clone()
{
	auto ret = ObjectFactory::Create<RemainUI>();
	return ret;
}

void ButiEngine::RemainUI::SetScore(const UINT arg_score)
{
	auto container = gameObject.lock()->GetResourceContainer();
	auto scorestr = std::to_string(arg_score);
	auto size = scorestr.size();
...
void ButiEngine::RemainUI::OnShowUI()
{
}
